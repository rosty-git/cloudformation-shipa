...
func Create(req handler.Request, prevModel *Model, currentModel *Model) (handler.ProgressEvent, error) {
	...
	job := convertModel(currentModel)
	raw, _ := json.Marshal(job)
	_, err = client.CreateJob(context.Background(), job)
	if err != nil {
...
func Read(req handler.Request, prevModel *Model, currentModel *Model) (handler.ProgressEvent, error) {
	...
	_, err = client.GetJobByName(context.Background(), *currentModel.Name)
	if err != nil {
		return handler.ProgressEvent{}, err
	}
...
func Update(req handler.Request, prevModel *Model, currentModel *Model) (handler.ProgressEvent, error) {
	...
	job, err := client.GetJobByName(context.Background(), *currentModel.Name)
	if err != nil {
		return handler.ProgressEvent{}, err
	}

	err = client.DeleteJob(context.Background(), job.ID)
	if err != nil {
		return handler.ProgressEvent{}, err
	}

	updatedJob := convertModel(currentModel)
	_, err = client.CreateJob(context.Background(), updatedJob)
	if err != nil {
		return handler.ProgressEvent{}, err
	}
